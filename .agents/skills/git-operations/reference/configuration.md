...
Never place credentials in remote URLs, config values, shell history, scripts, or logs. Use a credential helper appropriate to the host. For GitHub authentication, route to `github-operations`; do not print `gh auth token` or translate its output into a URL.
