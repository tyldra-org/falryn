...
| --- | --- |
| Open valid PR | Keep its issue, branch, PR, and valid companion PRs. Add focused implementation commits, then run a new Verify. |
| Closed without merge | Reopen only if its head, target, issue scope, and companions are still valid. Otherwise use a fresh branch and replacement PR. |
