...
Create and maintain GitHub pull requests. Use `git-operations` for fetch, branch, commit, push, rewrite, and checkout synchronization.
...
Resolve the repository, default branch, intended head, and current head SHA. Review the three-dot branch diff and commit range through `git-operations`; confirm the branch is pushed, current enough for repository policy, secret-free, and based on the intended branch.
...
`gh pr checkout` mutates local Git state, so also follow `git-operations`. Prefer a separate worktree or a deliberate detached checkout for untrusted review work:
...
Push focused follow-up commits through `git-operations`, then re-read the PR. A new head SHA invalidates earlier review and merge evidence.
...
Do not force-push a reviewed PR unless explicitly authorized through `git-operations`; a rewrite changes the evidence base.
