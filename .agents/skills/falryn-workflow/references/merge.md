...
Merge only when the user sent Merge — Target: PR #N (or Docs PR #N) and the
named PR has a fresh passing Verify that previewed the full delivery bundle,
ordering, exact squash subjects, optional footers, and available local
checkouts.

Before merging, re-read every PR head, required check, review, mergeability
result, companion, repository setting, previewed checkout, and default branch.
...
- Merge required docs companions first and the Falryn delivery PR last. Stop on
  the first failure and report the partial bundle exactly.
