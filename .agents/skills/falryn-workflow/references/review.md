...
Use `Review — Target: PR #N` (or `Docs PR #N`) to assess a pull request before
or alongside Verify. It is read-only: do not comment, approve, merge, change
...
1. Resolve the exact repository, PR, base and head SHA, author, linked issue,
   current check state, and any companion docs PR. Do not review a stale local
   branch as though it were the PR head.
