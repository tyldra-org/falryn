...
class PackageTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ('falryn-delivery', 'falryn-roadmap'):
    ...
    def test_link_to_sibling_skill_anchor(self):
        path = self.root / 'falryn-delivery/SKILL.md'
    ...
    def test_unreachable_reference_is_detected(self):
        (self.root / 'falryn-delivery/references/orphan.md').write_text('# Unreachable\n')
    ...
    def test_symlink_cannot_replace_a_skill(self):
        (self.root / 'falryn-roadmap/SKILL.md').unlink()
        (self.root / 'falryn-roadmap/SKILL.md').symlink_to(self.root / 'falryn-delivery/SKILL.md')
