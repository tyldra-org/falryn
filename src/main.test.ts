...
describe("application bootstrap", () => {
  ...
  test("opens the database in the state root and migrates it", async () => {
    ...
    // The production set is migrations 0001 through 0007, so a clean run ends
    ...
    expect(report.storage.ok && report.storage.value.appliedThisRun).toEqual([1, 2, 3, 4, 5, 6, 7]);
  });
