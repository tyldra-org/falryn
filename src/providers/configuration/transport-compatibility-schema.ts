import { processingQualificationsSchema } from "./processing.ts";
    ...
    .discriminatedUnion("dialect", [
      z
        .strictObject({
          ...
          processingQualifications: processingQualificationsSchema.default([]),
      ...
      z
        ...
        .superRefine((declaration, context) => {
          if (
            declaration.thinking === "adaptive" &&
            declaration.thinkingReplay !== "signed-blocks"
          ) {
            context.addIssue({
              code: "custom",
              path: ["thinkingReplay"],
      ...
      z
        .strictObject({
          ...
          usage: z
            .enum(GOOGLE_USAGE_MODES)
            .default(GOOGLE_GENERATE_CONTENT_TRANSPORT_DEFAULT.usage),
          inputEncoding: z
      ...
      z.strictObject({
        schemaVersion: version,
        processingQualifications: processingQualificationsSchema.default([]),
        dialect: z.literal("command-code-router"),
      }),
      z.strictObject({
        schemaVersion: version,
        processingQualifications: processingQualificationsSchema.default([]),
        dialect: z.literal("deterministic"),
      }),
      z.strictObject({
        schemaVersion: version,
        processingQualifications: processingQualificationsSchema.default([]),
        dialect: z.literal("openai-codex-unavailable"),
      }),
      z.strictObject({
        schemaVersion: version,
        processingQualifications: processingQualificationsSchema.default([]),
        dialect: z.literal("custom-unavailable"),
      }),
    ])
    .transform(({ processingQualifications, ...declaration }) =>
      processingQualifications.length === 0
        ? declaration
        : { ...declaration, processingQualifications },
    );
