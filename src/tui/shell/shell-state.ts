...
import { isContextActive } from "../commands/keymap.ts";
import {
  type ComposerAction,
  type ComposerState,
  composerReducer,
...
export function overlayRegions(route: OverlayRoute): readonly FocusRegion[] {
  switch (route.kind) {
    ...
    case "profile-result":
      return [{ id: "overlay.profile-result", label: "working profile result" }];
    case "model-settings":
      return [{ id: "overlay.model-settings", label: "model settings" }];
    case "compression":
