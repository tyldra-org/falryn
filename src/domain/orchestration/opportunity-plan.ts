...
import {
  ...
  type OpportunityModelAssistance,
  type OpportunityPlanInput,
  type OpportunityReasonCode,
  SOURCE_SCORE,
  STOP_WORDS,
  type TaskSignals,
} from "./opportunity-plan/contracts.ts";
import { degradationPlan, withTerminalReason } from "./opportunity-plan/degradation.ts";
...
export function tokens(value: string): ReadonlySet<string> {
  return new Set(
    value
      .toLocaleLowerCase()
      .split(/[^a-z0-9_.:/-]+/u)
...
export function decision(
  ...
  return Object.freeze({
    ...
    recoveryHandles: Object.freeze(
      health.diagnostics
        .flatMap((item) => (item.recovery === null ? [] : [item.recovery.handle]))
        .slice(0, MAX_OPPORTUNITY_REASON_CODES),
    ),
  });
}
...
export function primaryFamily(signals: TaskSignals): CapabilityFamily {
