...
describe("monotonic sequencing within a stream", () => {
  test("accepts events in order", () => {
    ...
    expect(sequencer.lastSequence(FIXTURE_STREAM)).toBe(sequence.from(12));
    expect(sequencer.expectedSequence(FIXTURE_STREAM)).toBe(sequence.from(13));
  });
...
describe("replay inspection", () => {
  test("reports a clean replay", () => {
    ...
    expect(report.appended).toBe(12);
