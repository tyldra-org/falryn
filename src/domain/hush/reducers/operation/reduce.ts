/** Safe operation projection over complete captures only. */
...
export function operationProjection(
  ...
  if (patterns.length > 0) {
    return plainTextProjection("operation", capture, maxBytes, patterns);
  }
  ...
  const source = [capture.stdout.inlineText, capture.stderr.inlineText]
    ...
    .map(stripAnsi)
    .join("\n");
  const formatted = formatOperationOutput(source, commandTokens);
