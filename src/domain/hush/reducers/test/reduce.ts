/** Safe test projection with exact fallback for unknown or partial output. */
...
export function testProjection(
  ...
  if (patterns.length > 0) {
    return plainTextProjection("test", capture, maxBytes, patterns);
  }
  ...
  const source = [capture.stdout.inlineText, capture.stderr.inlineText]
    ...
    .map(stripAnsi)
    .join("\n");
  const formatted = formatTestOutput(source, commandTokens);
