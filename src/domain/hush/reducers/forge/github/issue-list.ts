...
export function formatGithubIssueList(text: string): string | null {
  ...
  if (json !== null) {
    ...
    const lines = entries.map((entry) => {
      ...
      return `#${number} ${stateWord(state)} ${title}`;
    });
    return lines.every((line): line is string => line !== null) ? lines.join("\n") : null;
  }
  return formatNativeIssueList(text);
}

function formatNativeIssueList(text: string): string | null {
