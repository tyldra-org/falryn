...
import { duration, instant } from "../../../foundation/clock.ts";
import { processCaptureId } from "../../../foundation/index.ts";
import {
...
const EXPECTED_RULE_REDUCER_IDS = [
  "files.ls",
  "files.tree",
  "files.find",
  "files.read",
...
describe("Hush command rules", () => {
  test("keeps one ordered rule and reducer for every supported command family", () => {
    ...
    expect(ruleIds).toEqual([...EXPECTED_RULE_REDUCER_IDS]);
  ...
  test("covers every pinned RTK executable with an explicit policy", () => {
    const ruleExecutables = new Set(HUSH_COMMAND_RULES.flatMap((entry) => [...entry.executables]));
    const missing = PINNED_RTK_EXECUTABLES.filter((executable) => !ruleExecutables.has(executable));
    expect(missing).toEqual([]);
  });

  test("routes every rule example to its owning non-generic reducer", () => {
  ...
  test("runs every command rule as a recoverable projection", () => {
    ...
    for (const entry of HUSH_COMMAND_RULES) {
      ...
      expect(reduced.value.expansion.stdoutArtifact, example).toBe(artifactId.from("rules.stdout"));
    }
  });
});
...
function longReport(): ProcessCaptureReport {
  ...
  return {
    captureId: processCaptureId.from("rules"),
    pid: 42,
    ...
    stdout: {
      ...
      artifact: {
        artifactId: artifactId.from("rules.stdout"),
        committed: true,
        truncated: false,
