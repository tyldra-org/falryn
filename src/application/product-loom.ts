...
export type ProductLoomContext = {
  ...
  recoveryHandle(
    manifestId: string,
    projection?: LoomProjectionResult | null,
...
export function composeProductLoomContext(ports: ProductLoomContextPorts): ProductLoomContext {
  return {
    ...
    recoveryHandle(manifestId, projection = null) {
      return {
        owner: PRODUCT_LOOM_OWNER,
        manifestId,
        via: "loom-manifest",
        claimsExactSource: projection?.claimsExact === true,
      };
    },
    attachRecovery(payload, manifestId, projection = null) {
      return {
        ...payload,
