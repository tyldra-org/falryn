...
function run(
  mode: Mode,
  scenario: string,
  options: { readonly stdin?: string | null; readonly env?: Record<string, string> } = {},
): Run {
  ...
  const finished = Bun.spawnSync([...mode.command, scenario], {
...
for (const mode of [SOURCE, COMPILED]) {
  ...
  describe(`an interrupt in ${mode.name} mode`, () => {
    test(
      ...
      async () => {
        const child = Bun.spawn([...mode.command, "interrupt"], {
          env: probeEnvironment(),
          stdin: "ignore",
          stdout: "pipe",
          stderr: "pipe",
        ...
        // Waits for the probe's own readiness notice rather than for a fixed
        // delay, so an interrupt can never arrive before the first record.
        ...
        expect(child.exitCode).toBe(EXIT_CODES.CANCELLED);
      },
      PROBE_TIMEOUT_MS,
    );
  });
