...
async function worktrees(root: string) {
  ...
  // Directory symlinks need elevated privileges on Windows.
  if (process.platform !== "win32") await symlink(main, join(root, "link-a"));
...
test("one granted direction between worktrees succeeds across processes; everything else stays isolated", async () => {
  ...
  try {
    ...
    expect(
      refusal(await user({ operation: "route-grant", peer: alice.identity, expectedRevision: 0 })),
    ).toBe("ok");
    ...
    expect(fromA.value.items).toEqual(
      JSON.parse(JSON.stringify((fromB as { ok: true; value: { items: unknown[] } }).value.items)),
    );
