...
/**
 * What generation 5 produces for `everyEventKind()`.
 *
...
const GENERATION_9 = [
  ...
  {
    kind: "model-text",
    key: "declared:message:message-fixture",
    status: "final",
    outcome: null,
    disclosure: null,
  },
  ...
  ...["instructions.resolved", "instructions.revoked", "instructions.rejected"].map((kind) => ({
    kind: "notice",
    key: `declared:instructions:turn-fixture:${kind}`,
    status: "final",
    outcome: null,
    disclosure: null,
  })),
  {
    kind: "notice",
    key: "declared:schedule:attempt-fixture",
    status: "final",
    outcome: null,
    disclosure: null,
  },
...
describe("replaying the fixture run", () => {
  test("produces what this generation recorded", () => {
    expect(snapshot()).toEqual([...GENERATION_9]);
  });
  ...
  test("is the snapshot for the generation the build declares", () => {
    ...
    expect(TRANSCRIPT_PROJECTION_GENERATION).toBe(9);
  });
