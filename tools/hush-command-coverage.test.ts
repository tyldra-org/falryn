...
describe("Hush command coverage scorecard", () => {
  ...
  test("requires every maintained example to resolve to its owning reducer", () => {
    ...
    expect(scorecard.parityProvenProjections).toHaveLength(25);
    ...
    expect(scorecard.parityProvenReducers).toContain("data.json");
    expect(scorecard.parityProvenReducers).toContain("network.curl");
    expect(scorecard.parityProvenReducers).toContain("network.wget");
  });
  ...
  test("reports routing separately from token parity", () => {
    ...
    expect(formatted).toContain("token/context parity proven: 82 reducers across 25 projections");
  });
});
