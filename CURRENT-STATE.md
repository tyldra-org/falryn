...
Last reconciled: **2026-08-20** (#701 context/Brief/Hush/Loom/memory product wiring complete)
...
The session catalog slice from
[#258](https://github.com/tyldra-org/falryn/issues/258) lists, names, pins,
filters, and searches existing sessions without resuming a cursor or forking
history:

- `src/domain/session-catalog.ts` — `querySessionCatalog` and
  `editSessionCatalog` at version `session-catalog.v1`. Open/closed/pinned
  filters and title search stay on declared records. Pin is a catalog overlay
...
session from a durable event cursor:

- `src/domain/session-resume.ts` — `planSessionResume` at version
  `session-resume.v1`. A matching projection generation continues after the
  last applied sequence. A missing or stale-generation cursor rebuilds from
  the start of the same stream. A closed session is `closed`. A gap after
  the cursor is `malformed`. Provenance is `deterministic-event-cursors`
  with `model: null`.
- `src/application/session-resume.ts` — `resumeWorkspaceSession` loads the
  session, reads the next page through `EventStorePort.readFrom`, and never
  appends.

Validated by `src/domain/session-resume.test.ts` and
`src/application/session-resume.test.ts` under `bun run check`. There is no
command or OpenTUI overlay. Fork and rewind are #260. Replay controls remain #261.

The session rewind slice from
[#260](https://github.com/tyldra-org/falryn/issues/260) forks a new lineage
instead of rewriting history:

- `src/domain/session-rewind.ts` — `planSessionRewind` at version
  `session-rewind.v1`. Rewind points `parentTurnId` at a selected source
  turn. Fork copies the latest turn as checkpoint identity. Clone starts
  with an empty restore set. The source stream identity cannot be reused.
  Provenance is `deterministic-session-fork` with `model: null`.
- `src/application/session-rewind.ts` — `rewindWorkspaceSession` inserts the
  new session through `SessionRepositoryPort` and leaves the source row and
  stream unchanged.

Validated by `src/domain/session-rewind.test.ts` and
`src/application/session-rewind.test.ts` under `bun run check`. There is no
command or OpenTUI overlay. Replay controls remain #261.

The session replay-control slice from
[#261](https://github.com/tyldra-org/falryn/issues/261) moves a cursor over
recorded events without repeating effects:

- `src/domain/session-replay-control.ts` — `controlSessionReplay` at version
  `session-replay-control.v1`. Play, pause, step, and seek change only the
  cursor. Provenance is `deterministic-event-replay` with `model: null`.
- `src/application/session-replay-control.ts` — `controlWorkspaceSessionReplay`
  reads the session stream through `EventStorePort.readFrom` and never appends.

Validated by `src/domain/session-replay-control.test.ts` and
`src/application/session-replay-control.test.ts` under `bun run check`. There
is no command or OpenTUI overlay. Isolation remains #262.

The session isolation slice from
[#262](https://github.com/tyldra-org/falryn/issues/262) keeps an already-bound
workspace's sessions and warns when that binding's root or Git identity is
stale:

- `src/domain/session-isolation.ts` — `inspectSessionIsolation` at version
  `session-isolation.v1`. Foreign sessions are omitted. A changed root is
  `stale-root`. A changed Git identity is `stale-git`. This is not add-dir.
  Provenance is `deterministic-workspace-binding` with `model: null`.
- `src/application/session-isolation.ts` — `isolateWorkspaceSessions` lists
...
backup, inspect, restore, and diagnostics interactions without writing bytes:

- `src/domain/session-recovery.ts` — `planSessionRecovery` at version
  `session-recovery.v1`. Export builds a session selection. Import requires
  `verified: true`. Restore requires a closed live store and an exact
  confirmation bound to the backup name. Provenance is
  `deterministic-recovery-interaction` with `model: null`.
- `src/application/session-recovery.ts` — `planWorkspaceSessionRecovery`
  checks export scope against `SessionRepositoryPort` and fails closed on
  secret-shaped names.
...
The artifact command surface from
[#340](https://github.com/tyldra-org/falryn/issues/340) is implemented:

- `falryn artifact list`, `show`, and `get` on the yargs tree over
  `queryStoredArtifacts`, `createDurableArtifactApi`, and `createArtifactReader`.
- List is bounded (default 32, maximum 256) with truncation and an expansion
  route. Show reports not-found rather than empty success. Get streams bytes to
  `--output` or to stdout when stdout is not a terminal; restricted artifacts
  are not retrievable; no artifact body is inlined into a `CommandResult`.

Validated by `src/domain/artifact-catalog.test.ts`,
`src/cli/artifact-commands.test.ts`, and `bun run check`.

The over-bound machine refusal spill from
[#341](https://github.com/tyldra-org/falryn/issues/341) is implemented:

- An over-bound `falryn.cli` result is written to the artifact store; the
  `refusal` record carries `{ artifactId }` when the spill succeeds.
- When the store is unavailable or the write fails, the refusal still emits with
  `artifact: null` and `artifactError` (`store-unavailable` | `store-failed`).
- `CliResultRecord.artifacts` copies handles from `CommandResult.artifacts`.
- `falryn artifact get` retrieves spills larger than the default read chunk by
  raising range limits and writing file destinations without the stdout write
  bound.

Validated by `src/cli/schema.test.ts`, `src/cli/render-machine.test.ts`,
`src/cli/refusal-artifact.test.ts`, and `bun run check`.
...
printed a secret. `src/main.ts` now composes `composeProductCredentials` (#710)
so the shipped bootstrap retains the product secret resolver over keychain and
environment stores; `falryn run` resolves `FALRYN_OPENAI_API_KEY` through that
graph before attaching the OpenAI-compatible adapter.
...
The live composer slice from
[#612](https://github.com/tyldra-org/falryn/issues/612) wires that service into
the OpenTUI shell without reopening #249:

- `src/tui/mid-turn.ts` — composer-facing notices and draft policy (steer /
  follow-up clear; interrupt keeps the draft);
- `composer.submit` while a turn is active defaults to follow-up;
  `composer.submitAsSteer` / `composer.submitAsFollowUp` are explicit;
  `app.cancel` interrupts through the same service;
- status notices name steer, queued follow-up depth, or cancelling.

Validated by `src/tui/mid-turn.test.ts`,
`src/tui/components/live-composer-mid-turn.test.tsx`, and command availability
checks under `bun run check`.

The headless mid-turn JSONL slice from
[#613](https://github.com/tyldra-org/falryn/issues/613) projects classification
without a live composer:

- `src/domain/mid-turn-wire.ts` — wire form with queue order on every event;
- `src/cli/mid-turn.ts` — explicit-intent classify (fail closed), headless
  active-turn harness, and JSONL lifecycle + terminal records.

Validated by `src/cli/mid-turn.test.ts` under `bun run check`.

The hash-anchored patch hunk addressing slice from
[#614](https://github.com/tyldra-org/falryn/issues/614) extends the #66 patch
seam without reopening it:

- hunks may carry `addressDigest` (`sha-256:` of exact `oldLines`) and/or
  `id` / `hunkId` instead of restating `oldStart`;
- digest mismatch or a non-unique preimage match is a conflict (no relocate);
- apply still goes through `PatchPort` / `applyPatchHunks`.

Validated by `src/domain/workspace-patch.test.ts` under `bun run check`.
...
The workspace-set identity and per-root bind slice from
[#604](https://github.com/tyldra-org/falryn/issues/604) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) adds an ordered multi-root
set beside the #55 single-root binder:

- `WorkspaceRootId` in `src/domain/identity.ts`;
- `src/domain/workspace-set.ts` — `createWorkspaceSet` and `bindWorkspaceSetPath`
  with primary-first order, unique names, overlap/duplicate refusal, and
  `rootId` on every successful bind;
- `src/application/workspace-set.ts` — `resolveWorkspaceSet` (`realPath` +
  directory check) and `createWorkspaceSetBinder` (`symlink-escape` against the
  chosen root only);
- session isolation may carry optional `roots[]` and warn `stale-root` when the
  set drifts (`workspaceBindingFromSet`).

Validated by `src/domain/workspace-set.test.ts`,
`src/application/workspace-set.test.ts`, and updated
`src/domain/session-isolation.test.ts` under `bun run check`.

The named workspace-layout persistence slice from
[#605](https://github.com/tyldra-org/falryn/issues/605) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) stores
`{name, roots[]}` under the user configuration root:

- `src/domain/workspace-layout.ts` — layout name codec (profile rules),
  `workspace-layout.v1` documents, bounded catalog with expansion
  `workspace list --limit 256`;
- `src/application/workspace-layout.ts` — `createWorkspaceLayoutStore` save
  (force-required overwrite), load (typed `unusable-roots` naming every bad
  path), and list against `layouts/<name>.jsonc`.

Validated by `src/domain/workspace-layout.test.ts` and
`src/application/workspace-layout.test.ts` under `bun run check`.

The CLI and headless workspace-set slice from
[#606](https://github.com/tyldra-org/falryn/issues/606) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) exposes that set on the
yargs tree:

- `src/cli/workspace-resolution.ts` — `--workspace` path-or-layout plus
  repeatable `--add-dir`;
- `src/cli/command-tree.ts` / `commands.ts` / `dispatch.ts` /
  `render-human.ts` — `falryn workspace list|show|save|load` in all four
  output contracts; overwrite requires `--force`; help/version still
  construct no workspace scan.

Validated by `src/cli/workspace-commands.test.ts` and related CLI tests under
`bun run check`.

The TUI workspace-set commands and header slice from
[#607](https://github.com/tyldra-org/falryn/issues/607) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) wires the interactive
shell to the same set:

- `src/tui/workspace/` — header formatting (`falryn +2`) and
  `createWorkspaceController` over resolve/layout ports;
- palette commands `workspace.addRoot` / `removeRoot` / `save` / `load` /
  `show` (no default keys) open overlays; unavailable when no set is attached;
- launch path `ensureWorkspaceSet` attaches the controller and initial set.

Validated by `src/tui/workspace/format.test.ts`,
`src/tui/components/workspace-sheet.test.tsx`, and related shell tests under
`bun run check`.

The product workspace-set → language-server folder sync slice from
[#608](https://github.com/tyldra-org/falryn/issues/608) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) feeds the #90 seam:

- `src/domain/language-server-workspace.ts` — map roots to LSP folders, diff
  sets into `LanguageServerWorkspaceFoldersChange`, attribute a document URI to
  one `rootId`, and decide when configuration generation forces a resync;
- `src/application/language-server-workspace.ts` —
  `syncLanguageServerFoldersFromWorkspaceSet` applies the delta through
  `changeWorkspaceFolders`.

Validated by `src/domain/language-server-workspace.test.ts` and the product
folder sync case in `src/application/language-server.test.ts` under
`bun run check`.

The optional composer workspace slash aliases from
[#609](https://github.com/tyldra-org/falryn/issues/609) (parent
[#603](https://github.com/tyldra-org/falryn/issues/603)) map submit-time slash
text onto the same palette ids:

- `src/tui/composer/slash.ts` — `/workspace add|save|load|show` and doc forms
  `/add-dir` / `/save-workspace` / `/load-workspace`, with path or layout-name
  argument schema; unresolved `/workspace …` stays a notice, not a fake catalog;
- composer submit opens the same workspace overlays (prefilling add/save drafts)
  or loads a named layout; `composer.completion` remains reported as missing a
  completion popup.

Validated by `src/tui/composer/slash.test.ts` and composer slash cases in
`src/tui/components/workspace-sheet.test.tsx` under `bun run check`.
...
The durable memory record slice from
[#109](https://github.com/tyldra-org/falryn/issues/109) defines attributed,
correctable, versioned records without registering product tools, admitting
candidates under policy, ranking recall, or running operational learning:

- `src/domain/memory-record.ts` — `defineMemoryRecord` at version `memory.v1`.
  Records require provenance, a `MemoryId`, scope, kind, bounded subject and
  content, integer confidence 0–100, artifact sensitivity, and a canonical
  created time. Temporary inference and conversation origins are refused.
  Corrections are a new identity with `supersedes` links; the superseded
  record is not rewritten. Generation 1 has no lineage; lineage requires
  generation 2+. Cancellation, empty fields, oversized payloads, unknown
  schema versions, self-supersession, and review/expiry earlier than creation
  fail closed.
- `src/application/memory-record.ts` — `createMemoryRecords` keeps an
  in-memory map, refuses secret-shaped subject or content, and will not
  overwrite an existing id.

Validated by `src/domain/memory-record.test.ts` and
`src/application/memory-record.test.ts` under `bun run check`.

The memory admission slice from
[#110](https://github.com/tyldra-org/falryn/issues/110) applies context-governed
source, sensitivity, and workspace rules before a defined record is stored:

- `src/domain/memory-admission.ts` — `admitMemoryCandidate` at version
  `memory-admit.v1`. Repository and web sources cannot write. Inferred or
  untrusted trust is denied. Restricted sensitivity fails closed. User-wide
  scope requires a user-confirmed user source and public sensitivity.
  Project-local kinds cannot broaden to user/provider/collection scope.
  Workspace-scoped records must match the admitting workspace. Consolidation
  cannot broaden a sensitive prior. Cancellation fails closed.
- `src/application/memory-admission.ts` — `createMemoryAdmission` redacts
  secret-shaped candidates, then stores only admitted records.

Validated by `src/domain/memory-admission.test.ts` and
`src/application/memory-admission.test.ts` under `bun run check`.

The memory recall slice from
[#111](https://github.com/tyldra-org/falryn/issues/111) ranks stored records by
relevance, freshness, and confidence without registering product tools:

- `src/domain/memory-recall.ts` — `recallMemory` at version `memory-recall.v1`.
  Project-local records do not leak across workspaces. Expired and stale
  records are omitted. Destination sensitivity filters hits. A superseded
  identity is omitted; unresolved contradictions stay selected and are
  reported. Newer is not automatically truer. Rank-limit and cancellation fail
  closed.
- `src/application/memory-recall.ts` — `createMemoryRecall` reads the in-memory
  store through `list()` and does not echo omitted content.

Validated by `src/domain/memory-recall.test.ts` and
`src/application/memory-recall.test.ts` under `bun run check`.

The memory lifecycle slice from
[#112](https://github.com/tyldra-org/falryn/issues/112) corrects, deletes, and
expires records without rewriting canonical bytes or registering product tools:

- `src/domain/memory-lifecycle.ts` — `planMemoryCorrection`,
  `planMemoryDeletion`, and `planMemoryExpiry` at version
  `memory-lifecycle.v1`. A correction is a new identity with a later
  generation and a `supersedes` link. Deletion emits a receipt naming retained
  artifact or export locators. Expiry is an overlay; the stored record is not
  mutated. Cancellation fails closed.
- `src/application/memory-lifecycle.ts` — `createMemoryLifecycle` stores
  corrections through the existing record map, keeps deletion receipts and
  expiry overlays beside it, and exposes `listActive()` so recall omits
  deleted identities. Secret-shaped correction text fails closed.

Validated by `src/domain/memory-lifecycle.test.ts` and
`src/application/memory-lifecycle.test.ts` under `bun run check`.

The operational learning slice from
[#113](https://github.com/tyldra-org/falryn/issues/113) records bounded
aggregates and reviewable recommendations without converting telemetry into
durable facts or registering product tools:

- `src/domain/memory-learning.ts` — `defineOperationalObservation` and
  `defineOperationalRecommendation` at version `memory-learn.v1`. Observations
  carry error class, tool/provider identity, latency bucket, configuration
  generation, and outcome. Prompt, source, transcript, messages, content, and
  text fields are denied. Recommendations stay `suggested` and record
  supporting observations, counterexamples, expected benefit, risks, scope,
  and expiry. Cancellation fails closed.
- `src/application/memory-learning.ts` — `createOperationalLearning` stores
  those records in memory, requires supporting observations to exist, redacts
  secret-shaped text, and always denies `apply()`.

Validated by `src/domain/memory-learning.test.ts` and
`src/application/memory-learning.test.ts` under `bun run check`.

The memory isolation slice from
[#114](https://github.com/tyldra-org/falryn/issues/114) exports, replays, and
projects telemetry without leaking across workspaces or into telemetry text:

- `src/domain/memory-isolation.ts` — `projectMemoryExport` and
  `projectMemoryTelemetry` at version `memory-isolation.v1`. Export includes
  only the requested workspace (plus user-wide records with no workspace).
  Telemetry carries counts and kinds only. Cancellation fails closed.
- `src/application/memory-isolation.ts` — `createMemoryIsolation` admits
  through existing source rules, exports active records, replays a snapshot
  into a fresh map, and keeps repeated web/repository sources denied.

Validated by `src/domain/memory-isolation.test.ts` and
`src/application/memory-isolation.test.ts` under `bun run check`.
...
The bounded outcome-decomposition slice from
[#123](https://github.com/tyldra-org/falryn/issues/123) turns declared goals
into tasks without hidden scope growth, without registering a command or
calling a model:

- `src/domain/task-decompose.ts` — `decomposeUserOutcome` at version
  `task-decompose.v1`. One goal becomes one task when `proposed` is omitted.
  A proposed objective that is not an exact declared goal is `scope-growth`.
  Extra proposed fields are `malformed`. Partial splits report `omittedGoals`.
  Cancellation, empty statements, oversized goals, duplicate identities, and a
  present model identity fail closed. Provenance is `deterministic-goals` with
  `model: null`.
- `src/application/task-decompose.ts` — `decomposeOutcome` refuses
  secret-shaped statement or goal text, then returns the same advice. It does
  not execute Git, mutate files, or call a provider.

Validated by `src/domain/task-decompose.test.ts` and
`src/application/task-decompose.test.ts` under `bun run check`.

The static task-graph slice from
[#124](https://github.com/tyldra-org/falryn/issues/124) records dependencies,
blockers, joins, and completion criteria over already-bounded task identities,
without executing work or marking a task complete:

- `src/domain/task-graph.ts` — `planTaskGraph` at version `task-graph.v1`.
  Independent tasks are `ready`. A successor with predecessors is `waiting`.
  A declared external impediment is `blocked` even when predecessors exist.
  Join policy is `all` or `any` (default `all`). Cycles, unknown task ids,
  extra fields, empty task lists, cancellation, and a present model identity
  fail closed. Provenance is `deterministic-structure` with `model: null`.
- `src/application/task-graph.ts` — `planOutcomeTaskGraph` refuses
  secret-shaped blocker or criterion text, then returns the same advice.

Validated by `src/domain/task-graph.test.ts` and
`src/application/task-graph.test.ts` under `bun run check`.

The focused validation-advice slice from
[#125](https://github.com/tyldra-org/falryn/issues/125) recommends one
confirmation and one negative control per declared completion criterion,
without running tests or marking a task complete:

- `src/domain/task-validation.ts` — `recommendTaskValidation` at version
  `task-validation.v1`. Tasks without criteria are `omittedTasks`. Extra
  fields, empty task lists, cancellation, and a present model identity fail
  closed. Provenance is `deterministic-criteria` with `model: null`.
- `src/application/task-validation.ts` — `recommendOutcomeValidation` refuses
  secret-shaped criterion text, then returns the same advice.

Validated by `src/domain/task-validation.test.ts` and
`src/application/task-validation.test.ts` under `bun run check`.

The commit-plan product-wiring slice from
[#126](https://github.com/tyldra-org/falryn/issues/126) turns actual Git
changes into reviewable commit-plan advice, without staging or committing:

- `src/domain/task-commit-plan.ts` — `planTaskCommits` at version
  `task-commit-plan.v1`. It composes the #80 `planGitCommits` baseline.
  User-selected scope omits unrelated inventory rather than grouping it.
  A clean tree is `empty`. Extra fields, cancellation, and a present model
  identity fail closed. Provenance is `composed-git-status-log` with
  `model: null`.
- `src/application/task-commit-plan.ts` — `planOutcomeCommits` reads actual
  changes through `GitPort.planCommits`, refuses secret-shaped scope paths,
  and maps Git refusals to `unavailable`. It never stages or commits.

Validated by `src/domain/task-commit-plan.test.ts` and
`src/application/task-commit-plan.test.ts` under `bun run check`.

The progress-projection slice from
[#127](https://github.com/tyldra-org/falryn/issues/127) turns a static task
graph and observed facts into partial/complete state, recovery, and next
actions, without executing work or marking a task complete:

- `src/domain/task-progress.ts` — `projectTaskProgress` at version
  `task-progress.v1`. It composes the #124 graph. Observed completion that
  skips an unsatisfied join or a declared blocker is `stale`. Failed and
  cancelled tasks produce retry/resume advice. Provenance is
  `deterministic-observations` with `model: null`.
- `src/application/task-progress.ts` — `projectOutcomeProgress` refuses
  secret-shaped observation notes, blockers, and criteria, then returns the
  same advice.

Validated by `src/domain/task-progress.test.ts` and
`src/application/task-progress.test.ts` under `bun run check`.

The bounded review, advisor, and simplify slice from
[#285](https://github.com/tyldra-org/falryn/issues/285) runs one deterministic
inspection pass per mode without mutating files or calling a model:

- `src/domain/task-advisor.ts` — `adviseTask` at version `task-advisor.v1`.
  Review emits findings from declared excerpts. Advisor names rubric items
  missing from selected evidence. Simplify returns unapplied proposals.
  Extra mutation fields, empty required inputs, cancellation, and a present
  model identity fail closed. Provenance is `deterministic-inspection` with
  `model: null`.
- `src/application/task-advisor.ts` — `adviseOutcome` refuses secret-shaped
  question, evidence, rubric, or proposal text, then returns the same advice.

Validated by `src/domain/task-advisor.test.ts` and
`src/application/task-advisor.test.ts` under `bun run check`.
Qualified model-assisted drafting remains later and is not this slice.
...
- browser OAuth/PKCE and remote credential revocation productization beyond the
  existing truthful `not-attempted` rules (follow-on after #710);
- config file write/reload ([#704](https://github.com/tyldra-org/falryn/issues/704));
...
- a projection registry. One projection is maintained and its name is a closed
  ...
  not composed into a command. Session list, naming, pinning, filtering, and
  search exist as a catalog port
  ([#258](https://github.com/tyldra-org/falryn/issues/258)) and are not a
  command. Crash-safe resume from durable event cursors exists as a plan
  ([#259](https://github.com/tyldra-org/falryn/issues/259)) and is not a
  command. Session fork and rewind-as-new-history exist as a lineage plan
  ([#260](https://github.com/tyldra-org/falryn/issues/260)) and are not a
  command. Transcript replay controls exist as a cursor over recorded events
  ([#261](https://github.com/tyldra-org/falryn/issues/261)) and are not a
  command. User-facing rewind UX and reachability garbage collection remain
  [#257](https://github.com/tyldra-org/falryn/issues/257) and
...
- MCP, skills, plugin and other hook families, marketplace, agents, jobs, or
  workflows (deterministic goal decomposition exists for #123, a static
  task graph exists for #124, focused validation advice exists for #125, and
  commit-plan product wiring exists for #126, progress projection exists for
  #127, and bounded review/advisor/simplify advice exists for #285; they are
  not a command, model route, or orchestration graph); or
...
- **Current release outcome:** [v0.35 Live Product Coding Agent](https://github.com/tyldra-org/falryn/milestone/8) (product wiring). Parallel v0.4 UI look/feel: [#737](https://github.com/tyldra-org/falryn/issues/737).
...
- **Completed Brief, Hush, Loom parent:** [#101 Implement Brief, Hush, Loom, and compression](https://github.com/tyldra-org/falryn/issues/101) is closed and Done. Child [#102 Implement Brief as response-style projection without evidence loss](https://github.com/tyldra-org/falryn/issues/102) closed and Done via [PR #632](https://github.com/tyldra-org/falryn/pull/632) (`8b669b3`); companion docs [falryn-docs#193](https://github.com/tyldra-org/falryn-docs/pull/193) (`3828da9`). Child [#103 Integrate Hush across shell, Git, tests, search, and process output](https://github.com/tyldra-org/falryn/issues/103) closed and Done via [PR #634](https://github.com/tyldra-org/falryn/pull/634) (`1ca4435`); companion docs [falryn-docs#194](https://github.com/tyldra-org/falryn-docs/pull/194) (`7fbdd07`). Child [#104 Implement Loom compress-cache-retrieve manifests and exact retrieval](https://github.com/tyldra-org/falryn/issues/104) closed and Done via [PR #636](https://github.com/tyldra-org/falryn/pull/636) (`20595b7`); companion docs [falryn-docs#195](https://github.com/tyldra-org/falryn-docs/pull/195) (`af87619`). Child [#105 Implement structural reducers for files, diffs, diagnostics, and tools](https://github.com/tyldra-org/falryn/issues/105) closed and Done via [PR #638](https://github.com/tyldra-org/falryn/pull/638) (`20523ec`); companion docs [falryn-docs#196](https://github.com/tyldra-org/falryn-docs/pull/196) (`ffa223a`). Child [#106 Implement optional compact-model and history checkpoint lanes](https://github.com/tyldra-org/falryn/issues/106) closed and Done via [PR #641](https://github.com/tyldra-org/falryn/pull/641) (`14c2b96`); companion docs [falryn-docs#199](https://github.com/tyldra-org/falryn-docs/pull/199) (`9bf5ab9`). Child [#107 Evaluate fidelity, reversibility, latency, and token savings](https://github.com/tyldra-org/falryn/issues/107) closed and Done via [PR #643](https://github.com/tyldra-org/falryn/pull/643) (`8f0d431`); companion docs [falryn-docs#200](https://github.com/tyldra-org/falryn-docs/pull/200) (`adc988f`). Parent integrated verification passed on `8f0d431` (1 child-seam test).
- **Completed memory parent:** [#108 Implement durable memory and operational learning](https://github.com/tyldra-org/falryn/issues/108) is closed and Done. Parent integrated verification passed on `5ed2e03` (64 child-seam tests). Child [#109 Define attributed, correctable, versioned memory records](https://github.com/tyldra-org/falryn/issues/109) closed and Done via [PR #645](https://github.com/tyldra-org/falryn/pull/645) (`f5fd6d2`); companion docs [falryn-docs#201](https://github.com/tyldra-org/falryn-docs/pull/201) (`29cf0bd`). Child [#110 Implement context-governed memory admission and sensitivity rules](https://github.com/tyldra-org/falryn/issues/110) closed and Done via [PR #646](https://github.com/tyldra-org/falryn/pull/646) (`9656324`); companion docs [falryn-docs#202](https://github.com/tyldra-org/falryn-docs/pull/202) (`1b34a90`). Child [#111 Implement memory recall, ranking, freshness, and contradiction handling](https://github.com/tyldra-org/falryn/issues/111) closed and Done via [PR #647](https://github.com/tyldra-org/falryn/pull/647) (`bb51ed4`); companion docs [falryn-docs#203](https://github.com/tyldra-org/falryn-docs/pull/203) (`9d6c7d5`). Child [#112 Implement correction, supersession, deletion, and retention](https://github.com/tyldra-org/falryn/issues/112) closed and Done via [PR #648](https://github.com/tyldra-org/falryn/pull/648) (`fc3a19d`); companion docs [falryn-docs#204](https://github.com/tyldra-org/falryn-docs/pull/204) (`33079b1`). Child [#113 Implement operational learning without converting telemetry into truth](https://github.com/tyldra-org/falryn/issues/113) closed and Done via [PR #649](https://github.com/tyldra-org/falryn/pull/649) (`66a6d18`); companion docs [falryn-docs#205](https://github.com/tyldra-org/falryn-docs/pull/205) (`baa0c5d`). Child [#114 Add privacy, export, replay, and cross-workspace isolation tests](https://github.com/tyldra-org/falryn/issues/114) closed and Done via [PR #650](https://github.com/tyldra-org/falryn/pull/650) (`5ed2e03`); companion docs [falryn-docs#206](https://github.com/tyldra-org/falryn-docs/pull/206) (`e03bd55`).
- **Completed artifacts, export, replay, and recovery parent:** [#115 Complete artifacts, export, replay, and recovery](https://github.com/tyldra-org/falryn/issues/115) is closed and Done. Parent integrated verification passed on `e42b1b0` (313 child-seam tests). Child [#116 Complete the typed artifact API, provenance, integrity, and lifecycle](https://github.com/tyldra-org/falryn/issues/116) closed and Done via [PR #653](https://github.com/tyldra-org/falryn/pull/653) (`bbf3337`); companion docs [falryn-docs#207](https://github.com/tyldra-org/falryn-docs/pull/207) (`2078302`). Child [#117 Implement code, diff, document, media, and diagnostic viewers](https://github.com/tyldra-org/falryn/issues/117) closed and Done via [PR #654](https://github.com/tyldra-org/falryn/pull/654) (`fa2b715`); companion docs [falryn-docs#208](https://github.com/tyldra-org/falryn-docs/pull/208) (`4253343`). Child [#118 Implement versioned export bundles with redaction and manifests](https://github.com/tyldra-org/falryn/issues/118) closed and Done via [PR #655](https://github.com/tyldra-org/falryn/pull/655) (`a043b76`); companion docs [falryn-docs#209](https://github.com/tyldra-org/falryn-docs/pull/209) (`efc34dd`). Child [#119 Implement deterministic effect-free session and artifact replay](https://github.com/tyldra-org/falryn/issues/119) closed and Done via [PR #656](https://github.com/tyldra-org/falryn/pull/656) (`1740a7b`); companion docs [falryn-docs#210](https://github.com/tyldra-org/falryn-docs/pull/210) (`62fdbb2`). Child [#120 Recover from interrupted writes, corruption, missing blobs, and schema drift](https://github.com/tyldra-org/falryn/issues/120) closed and Done via [PR #657](https://github.com/tyldra-org/falryn/pull/657) (`2c52421`); companion docs [falryn-docs#211](https://github.com/tyldra-org/falryn-docs/pull/211) (`4a44306`). Child [#121 Implement backup, restore, retention, cleanup, and support diagnostics](https://github.com/tyldra-org/falryn/issues/121) closed and Done via [PR #658](https://github.com/tyldra-org/falryn/pull/658) (`e42b1b0`); companion docs [falryn-docs#212](https://github.com/tyldra-org/falryn-docs/pull/212) (`14d43d4`).
- **Completed task intelligence parent:** [#122 Implement task intelligence and commit planning](https://github.com/tyldra-org/falryn/issues/122) is closed and Done. Parent integrated verification passed on `6e6ad2b` (59 child-seam tests). Child [#123 Decompose user outcomes into bounded tasks without hidden scope growth](https://github.com/tyldra-org/falryn/issues/123) closed and Done via [PR #660](https://github.com/tyldra-org/falryn/pull/660) (`306d4a6`); companion docs [falryn-docs#213](https://github.com/tyldra-org/falryn-docs/pull/213) (`ac45108`). Child [#124 Represent dependencies, blockers, joins, and completion criteria](https://github.com/tyldra-org/falryn/issues/124) closed and Done via [PR #661](https://github.com/tyldra-org/falryn/pull/661) (`480e7eb`); companion docs [falryn-docs#214](https://github.com/tyldra-org/falryn-docs/pull/214) (`94745ca`). Child [#125 Recommend focused validation and negative controls](https://github.com/tyldra-org/falryn/issues/125) closed and Done via [PR #662](https://github.com/tyldra-org/falryn/pull/662) (`fe4b80f`); companion docs [falryn-docs#215](https://github.com/tyldra-org/falryn-docs/pull/215) (`3467798`). Child [#126 Generate reviewable commit plans from actual changes](https://github.com/tyldra-org/falryn/issues/126) closed and Done via [PR #663](https://github.com/tyldra-org/falryn/pull/663) (`1c2648b`); companion docs [falryn-docs#216](https://github.com/tyldra-org/falryn-docs/pull/216) (`0a9d247`). Child [#127 Project progress, partial completion, recovery, and next actions](https://github.com/tyldra-org/falryn/issues/127) closed and Done via [PR #664](https://github.com/tyldra-org/falryn/pull/664) (`46153c8`); companion docs [falryn-docs#217](https://github.com/tyldra-org/falryn-docs/pull/217) (`de105a1`). Child [#285 Implement bounded review, advisor, and simplify modes](https://github.com/tyldra-org/falryn/issues/285) closed and Done via [PR #665](https://github.com/tyldra-org/falryn/pull/665) (`6e6ad2b`); companion docs [falryn-docs#218](https://github.com/tyldra-org/falryn-docs/pull/218) (`5f31984`).
- **Completed session navigation parent:** [#257 Deliver session navigation, resume, fork, and rewind](https://github.com/tyldra-org/falryn/issues/257) is closed and Done. Parent integrated verification passed on `1fccd54` (4 child-seam tests in `src/domain/session-navigation.test.ts`). Child [#258 Implement session list, naming, pinning, filtering, and search](https://github.com/tyldra-org/falryn/issues/258) closed and Done via [PR #668](https://github.com/tyldra-org/falryn/pull/668) (`e2787c8`); companion docs [falryn-docs#220](https://github.com/tyldra-org/falryn-docs/pull/220) (`4bad586`). Child [#259 Implement crash-safe resume from durable event cursors](https://github.com/tyldra-org/falryn/issues/259) closed and Done via [PR #669](https://github.com/tyldra-org/falryn/pull/669) (`93f2a17`); companion docs [falryn-docs#221](https://github.com/tyldra-org/falryn-docs/pull/221) (`642db46`). Child [#260 Implement session fork and rewind-as-new-history semantics](https://github.com/tyldra-org/falryn/issues/260) closed and Done via [PR #670](https://github.com/tyldra-org/falryn/pull/670) (`dd5c4ec`); companion docs [falryn-docs#222](https://github.com/tyldra-org/falryn-docs/pull/222) (`37403ea`). Child [#261 Implement transcript replay controls without repeating effects](https://github.com/tyldra-org/falryn/issues/261) closed and Done via [PR #671](https://github.com/tyldra-org/falryn/pull/671) (`d04d0b7`); companion docs [falryn-docs#223](https://github.com/tyldra-org/falryn-docs/pull/223) (`300dd57`). Child [#262 Implement workspace and session isolation with stale-source warnings](https://github.com/tyldra-org/falryn/issues/262) closed and Done via [PR #672](https://github.com/tyldra-org/falryn/pull/672) (`d7de659`); companion docs [falryn-docs#224](https://github.com/tyldra-org/falryn-docs/pull/224) (`11ce7eb`). Child [#263 Implement session export, import, and recovery interactions](https://github.com/tyldra-org/falryn/issues/263) closed and Done via [PR #673](https://github.com/tyldra-org/falryn/pull/673) (`1fccd54`); companion docs [falryn-docs#225](https://github.com/tyldra-org/falryn-docs/pull/225) (`00d35bc`).
- **Completed multi-root workspaces parent:** [#603 Deliver multi-root workspaces and named layouts](https://github.com/tyldra-org/falryn/issues/603) is closed and Done. Parent integrated verification passed on `389a2f7` (62 child-seam tests across workspace-set, layout, CLI, TUI sheets, LSP folder sync, and composer slash aliases). Child [#604](https://github.com/tyldra-org/falryn/issues/604) via [PR #691](https://github.com/tyldra-org/falryn/pull/691); [#605](https://github.com/tyldra-org/falryn/issues/605) via [PR #692](https://github.com/tyldra-org/falryn/pull/692); [#606](https://github.com/tyldra-org/falryn/issues/606) via [PR #693](https://github.com/tyldra-org/falryn/pull/693); [#607](https://github.com/tyldra-org/falryn/issues/607) via [PR #694](https://github.com/tyldra-org/falryn/pull/694); [#608](https://github.com/tyldra-org/falryn/issues/608) via [PR #695](https://github.com/tyldra-org/falryn/pull/695); [#609](https://github.com/tyldra-org/falryn/issues/609) via [PR #696](https://github.com/tyldra-org/falryn/pull/696) (`389a2f7`); docs companions [falryn-docs#240](https://github.com/tyldra-org/falryn-docs/pull/240)–[#245](https://github.com/tyldra-org/falryn-docs/pull/245).
- **New productization milestone:** [v0.35 Live Product Coding Agent](https://github.com/tyldra-org/falryn/milestone/8) owns the orphan product-wiring layer (libraries existed; product path did not). Parents: [#699 live coding agent](https://github.com/tyldra-org/falryn/issues/699), [#700 built-in tools](https://github.com/tyldra-org/falryn/issues/700), [#701 context/Brief/Hush/Loom/memory product wiring](https://github.com/tyldra-org/falryn/issues/701), [#702 session/data commands](https://github.com/tyldra-org/falryn/issues/702), [#703 task intelligence surfaces](https://github.com/tyldra-org/falryn/issues/703), [#704 config/runtime polish](https://github.com/tyldra-org/falryn/issues/704). Same-milestone manual optimize track: [#736 Brief vs Caveman / Hush vs rtk / Loom vs Headroom](https://github.com/tyldra-org/falryn/issues/736) (after #717–#719). v0.4 [#128 MCP](https://github.com/tyldra-org/falryn/issues/128) is blocked by #699/#700; [#210](https://github.com/tyldra-org/falryn/issues/210) measures Hush/Loom after #701.
- **Completed live coding agent parent:** [#699 Deliver the live coding agent end-to-end](https://github.com/tyldra-org/falryn/issues/699) is closed and Done. Parent integrated verification passed on `61aabf9` (child-seam suites for product runtime, credentials, coding-run, OpenAI-compatible adapter, product submission, and `composeProductShellAttachments`). Children [#705](https://github.com/tyldra-org/falryn/issues/705)–[#710](https://github.com/tyldra-org/falryn/issues/710) and [#752](https://github.com/tyldra-org/falryn/issues/752) closed.
- **Completed built-in tools parent:** [#700 Register built-in workspace and coding tools](https://github.com/tyldra-org/falryn/issues/700) closed after children [#711](https://github.com/tyldra-org/falryn/issues/711)–[#714](https://github.com/tyldra-org/falryn/issues/714) registered filesystem/reader/search/patch, process/shell, Git/worktree, and LSP/DAP product tools through `ToolRunnerPort`.
- **Completed context/compression parent:** [#701 Wire context, Brief, Hush, Loom, and memory into live turns](https://github.com/tyldra-org/falryn/issues/701) closed after children [#715](https://github.com/tyldra-org/falryn/issues/715)–[#720](https://github.com/tyldra-org/falryn/issues/720).
- **Completed session/data product commands parent:** [#702 Complete session navigation and data lifecycle product commands](https://github.com/tyldra-org/falryn/issues/702) closed after children [#721](https://github.com/tyldra-org/falryn/issues/721)–[#725](https://github.com/tyldra-org/falryn/issues/725): session resume/fork/rewind/replay CLI + OpenTUI overlays, import/`falryn replay`, data backup/restore/inspect/diagnostics, and retention/reachability GC. Replay stays effect-free; restore/GC are preview-first with confirmation.
- **Completed task intelligence product surfaces parent:** [#703 Deliver task intelligence product surfaces](https://github.com/tyldra-org/falryn/issues/703) closed after children [#726](https://github.com/tyldra-org/falryn/issues/726)–[#727](https://github.com/tyldra-org/falryn/issues/727): `falryn task decompose|validate|progress|commit-plan` and matching OpenTUI overlays. Commit-plan apply requires an exact `plan-commit-…` confirm token for the refreshed plan.
- **Completed configuration and runtime polish parent:** [#704 Complete configuration and runtime product polish](https://github.com/tyldra-org/falryn/issues/704) closed after children [#728](https://github.com/tyldra-org/falryn/issues/728)–[#731](https://github.com/tyldra-org/falryn/issues/731): product bootstrap binds loader generations, `config set` + live reload, owned-process shutdown participant, and shell completion for declared commands.
- **TUI visual language (v0.4):** [#737 Deliver Falryn TUI visual language](https://github.com/tyldra-org/falryn/issues/737) (Claude-minimal half-splits, Grok-interactive focus mode, Command-editorial badges) with children [#738](https://github.com/tyldra-org/falryn/issues/738)–[#743](https://github.com/tyldra-org/falryn/issues/743). Does not reopen #21/#249. [#271 settings/extensions/dashboards](https://github.com/tyldra-org/falryn/issues/271) keeps feature ownership and reuses the #737 kit via #743.
- **Next planning action:** start same-milestone [#736](https://github.com/tyldra-org/falryn/issues/736) (Brief/Hush/Loom optimize) or v0.4 [#737](https://github.com/tyldra-org/falryn/issues/737) TUI visual language per Roadmap priority. GitHub Project and Roadmap remain authoritative.