...
- **Active live coding agent parent:** [#699 Deliver the live coding agent end-to-end](https://github.com/tyldra-org/falryn/issues/699) is In Progress. Child [#705 Compose the agent turn loop, provider session, and durable turn journal in product bootstrap](https://github.com/tyldra-org/falryn/issues/705) lands `composeProductAgentRuntime` in `src/application/product-agent-runtime.ts` (fail-closed host with coordinator + journal + attachment seams; empty tools; no live vendors). Remaining children: [#706](https://github.com/tyldra-org/falryn/issues/706)–[#710](https://github.com/tyldra-org/falryn/issues/710).
